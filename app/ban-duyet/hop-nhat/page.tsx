 ...
 * và core/tree (getClanOverview cho mảnh, getAncestryPath cho đời + chi của từng ứng viên).
 *
...
import { ThaoTacDeXuat } from './thao-tac-de-xuat';
 ...
 * tự tra.
 */
function soSanh(c: DuplicateCandidate, theA: PersonCard | undefined, theB: PersonCard | undefined) {
  const giong: string[] = [];
  const khac: string[] = [];
  const e = c.evidence;
...
export default async function Page() {
  ...
  const ids = [...new Set(cap.flatMap((c) => [c.a.personId, c.b.personId]))];
  ...
  return (
    <main className="mx-auto max-w-[1100px] px-6 py-10">
      ...
      {cap.length === 0 ? (
        ...
        <div className="mt-5 grid gap-5">
          {cap.map((c) => {
            ...
            const { giong, khac } = soSanh(c, theA, theB);
            return (
              <div
      ...
      {/* ── Khu (b): đề xuất đang mở · gộp và tách lại ───────────────────────────────────── */}
      <h2 className="mt-12 text-[19px] font-semibold">Đề xuất đang mở · gộp và tách lại</h2>
      {/* TODO(core): core/merge chưa có API liệt kê đề xuất đang mở và lịch sử gộp gần đây
          (cần listProposals()/listMergeHistory()) — nên khu này chưa bày được danh sách.
          Thao tác tạm nhận mã đề xuất; có API thì thay bằng danh sách thật + chi tiết cặp. */}
      <p className="mt-1.5 max-w-[70ch] text-[17px] text-muted-foreground">
        Danh sách đề xuất chưa bày ra được ở đây (core chưa có đường liệt kê — đã ghi nợ). Mỗi
        đề xuất có một mã, hiện ra ngay lúc mở; thao tác theo mã ở dưới. Gộp chạy trong một
        lần ghi duy nhất và ghi trọn danh sách mối nối, nên tách lại là tách đúng.
      </p>
      <div className="mt-5">
        <ThaoTacDeXuat />
      </div>
