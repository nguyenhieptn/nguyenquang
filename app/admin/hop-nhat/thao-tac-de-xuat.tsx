...
export function ThaoTacDeXuat({ proposalId }: { proposalId: string }) {
  ...
  const gop = () =>
    batDau(async () => {
      let ketQua: Awaited<ReturnType<typeof gopDeXuat>>;
      try {
        ketQua = await gopDeXuat(proposalId);
      } catch {
        // Promise reject TRONG một transition không tới `error.tsx` — React đẩy nó ra
        // `reportGlobalError`, tức ra console và không đâu khác. Người vận hành chỉ thấy nút
        // nhấp nháy rồi thôi, trên đúng màn có phép gộp không hoàn tác được bằng tay.
        setThongBao({ loai: 'loi', cau: 'Không gửi được lên máy chủ. Kiểm tra mạng rồi thử lại.' });
        return;
      }
      if (ketQua.ok) {
        setThongBao({
  ...
  const bac = () =>
    batDau(async () => {
      let ketQua: Awaited<ReturnType<typeof khongPhaiMotNguoi>>;
      try {
        ketQua = await khongPhaiMotNguoi(proposalId, lyDo);
      } catch {
        // Promise reject TRONG một transition không tới `error.tsx` — React đẩy nó ra
        // `reportGlobalError`, tức ra console và không đâu khác. Người vận hành chỉ thấy nút
        // nhấp nháy rồi thôi, trên đúng màn có phép gộp không hoàn tác được bằng tay.
...
export function TachLaiNut({ proposalId }: { proposalId: string }) {
  ...
  const tach = () =>
    batDau(async () => {
      let ketQua: Awaited<ReturnType<typeof tachLai>>;
      try {
        ketQua = await tachLai(proposalId);
      } catch {
        // Promise reject TRONG một transition không tới `error.tsx` — React đẩy nó ra
        // `reportGlobalError`, tức ra console và không đâu khác. Người vận hành chỉ thấy nút
        // nhấp nháy rồi thôi, trên đúng màn có phép gộp không hoàn tác được bằng tay.
