...
export function BangChoDuyet({ dong }: { dong: DongChoDuyet[] }) {
  ...
  const nangLoat = () =>
    batDauNangLoat(async () => {
      ...
      try {
        ketQua = await duyetHangLoat([...daChon]);
      } catch {
        ...
        setKetQuaLoat({ daNang: 0, loi: ['Không gửi được lên máy chủ. Kiểm tra mạng rồi thử lại.'] });
        return;
      }
      if (ketQua.ok) {
    ...
    <TableRow>
      ...
      <TableCell className="align-top">
        {/* Dữ liệu phả giữ nguyên luật bề mặt A ngay giữa khung trần: serif-phả, nét đứt +
            vân tồn nghi. Bàn làm việc vẽ khác thì người vận hành duyệt một thứ và người trong
