# React Key Screens Renderer (Gia phả Nguyễn Quang override)

Prompt cho subagent. Thay renderer HTML mặc định (`assets/key-screens.md`) khi làm UX cho
Gia phả Nguyễn Quang. Bắn ở Finalize (hoặc cuối Discovery khi layout đã chắc). Sinh **page React 1:1 trong
...
- **Dùng primitive có sẵn** từ `@/components/ui/*`. Repo mới cắm nên hiện **chỉ có `card`** —
  cần thêm primitive thì chạy `npx shadcn@latest add <tên>` (base `radix`, preset `nova`) rồi
  dùng, đừng tự dựng thẻ mới ngoài khuôn. Cần biến thể chưa có → thêm vào `variants` của chính
  primitive đó, hoặc ghi vào spine + hỏi.
  > `add card` sẽ ghi đè `components/ui/card.tsx` và **xoá alias export `CardBody`** mà plumbing
  > xưởng cần — thêm lại `CardContent as CardBody` sau mỗi lần chạy.
- **Tĩnh, mock-only, không phụ thuộc hạ tầng:** server component (mặc định), KHÔNG import
  `@/core/*`, `@/db/*`, Drizzle, Better Auth, server action, hay `next/navigation`
  (notFound/redirect). Form là hình ảnh (không `action`, nút `type="button"`).
  Có vậy màn mới mở được trong khu làm việc mà không cần DB seed / đăng nhập.
  *(Đây cũng chính là AD-1 của spine: `app/` không chạm tầng dữ liệu.)*
- **Token có tên**, không hardcode hex: gọi token **brand** của project. Nguồn là `@theme` trong
  `app/globals.css` (bắt nguồn từ DESIGN.md). **Đừng gọi `ws-*`** — bảng đó là màu của cái vỏ
  xưởng, không phải của sản phẩm (xem `specs/frontend-stack.md` §2).
...
> tương ứng, họ **promote** (di chuyển ra route thật, thay `_mock/seed` bằng lời gọi `core/`, giữ nguyên
