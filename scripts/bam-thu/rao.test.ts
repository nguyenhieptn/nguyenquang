...
describe('ba rào của kịch bản ghi (story 7-1)', () => {
  ...
  it('rào tài khoản: chỉ thu.quan.tri.* / thu.thanh.vien.*; chưa gắn và tài khoản thật đều dừng', () => {
    ...
    expect(raoTaiKhoan('thu.thanh.vien.0b9b84').ok).toBe(true);
  ...
  it('rào thanh trên: phải mang họ thử', () => {
    expect(raoThanhTren('T0b9b84 Nguyễn Thử Quản Trị · quản trị').ok).toBe(true);
    expect(raoThanhTren('Nguyễn Quang Hiệp · quản trị')).toMatchObject({ ok: false, rao: 'thanh-tren' });
  });
